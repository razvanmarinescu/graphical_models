...
avg_err
save('p27_6_errors2.mat', 'avg_err')

end

function error = sampleHMM(lambda)
...
H=2; V=2; T=10;
% make a HMM
% condp make a conditional probability table by normalising the
% probabilities. for small lambda (0.1) the probs end up being equal, while
% for big lambda (20), the probabilities are highly unequal
...
error = mean(abs(gamma(:)-gamma_samp(:)));
